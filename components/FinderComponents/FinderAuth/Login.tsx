...
export default function Login(props: any) {
	const { setUser, setFinderLoginModal } = useContext(AppContext);
	...
	const [fetchUserData, { data: userData }] = useLazyQuery(GET_USER_BY_ID, {
		onCompleted() {
			console.log(userData.core_users_by_pk);
			setUser({ ...userData.core_users_by_pk });
			setFinderLoginModal(false);
			return;
		},
		onError(error) {
			console.log(error);
	...
	return (
		<form onSubmit={handleSubmit(onLogin)}>
			...
			<button
				type="submit"
				className="my-5 mx-auto block bg-red text-white text-center py-3 px-8 w-full rounded-md"
			>
				Sign In
			</button>
			{errorMessage && (
