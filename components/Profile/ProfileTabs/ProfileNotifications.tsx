...
const ProfileNotifications = ({ finder }: { finder?: boolean }) => {
		...
		<section className="w-full mx-auto px-5 py-4 my-3">
			<div className="grid grid-cols-1 md:grid-cols-6 gap-3">
				<div className="col-span-1 md:col-span-2">
					...
					<form
						className="my-5 mr-12"
						onSubmit={handleSubmit(createAlertHandler)}
					>
						...
						<div className="my-8 relative flex justify-end items-start">
							<button
								className={clsx(
									finder ? 'btn-fnd-primary' : 'btn-primary',
									'm-0 py-2 px-16 font-medium text-xl rounded-md shadow-md'
								)}
								type="submit"
							>
								Add Alert
							...
							<tbody>
								{alerts.map((alert) => (
									<tr key={alert.id}>
										...
										<td>{alert.city}</td>
										<td>
											<div className={styles.propTypeChip}>
												{alert.prop_type}
											</div>
											<p className="text-center" style={{ color: '#6E6893' }}>
												Property Types
											</p>
										</td>
										<td>
										...
										<td>
											<button
												className="bg-transparent border-transparent mx-auto cursor-pointer"
