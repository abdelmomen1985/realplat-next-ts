import React, { useState, useContext } from 'react';
import { GetStaticPaths, GetStaticProps } from 'next';
import ProfileHero from '../../../../components/Profile/ProfileHero';
import { getLocalizationProps } from './../../../../Context/LangContext';
import ProfileTabs from './../../../../components/Profile/ProfileTabs';
import AccountSettings from './../../../../components/Profile/ProfileTabs/AccountSettings';
import PersonalInfo from './../../../../components/Profile/ProfileTabs/PersonalInfo';
import ProfileNotifications from './../../../../components/Profile/ProfileTabs/ProfileNotifications';
import { AppContext } from './../../../../Context/AppContextProvider';
import FinderLayout from './../../../../components/Layouts/FinderLayout';

const UserSettingsPage = () => {
	const [currentTap, setCurrentTap] = useState<number>(1);
	// const { user } = useContext(AppContext);
	return (
		<FinderLayout>
			<ProfileHero finder />
			<ProfileTabs finder currentTap={currentTap} setCurrentTap={setCurrentTap}>
				{currentTap === 1 && <AccountSettings finder />}
				{currentTap === 2 && <PersonalInfo finder />}
				{/* {currentTap === 3 && <AccountSettings />} */}
				{currentTap === 4 && <ProfileNotifications finder />}
			</ProfileTabs>
		</FinderLayout>
	);
};

export default UserSettingsPage;
export const getStaticProps: GetStaticProps = async (ctx) => {
	// Example for including static props in a Next.js function component page.
	// Don't forget to include the respective types for any props passed into
	// the component.
	const localization = getLocalizationProps(ctx, 'common');
	return {
		props: {
			localization,
		},
	};
};

export const getStaticPaths: GetStaticPaths = async () => {
	return {
		paths: ['en', 'ar'].map((lang) => ({ params: { lang } })),
		fallback: false,
	};
};
